...
> **Stav k 17. 9. 2026:** na VPS zatím **nic z tohoto projektu
> neexistuje**. Sekce označené *(plán)* popisují, co se má vytvořit —
> ne co je ověřeno. Po prvním úspěšném deployi se přepíšou podle
> skutečnosti.
...
## 2. Izolace od ostatních projektů

Na tomtéž VPS běží nezávisle **DSS**, **Evidence nářadí**, **WordPress**
a případně další. Deploy Knihy jízd se jich nesmí dotknout.
...
- Persistentní data: pojmenovaný volume `kniha_jizd_pgdata` pro databázi,
  bind-mounty pro `photos/` a `documents/`.
...
- Aplikace se připojuje vlastním DB uživatelem (`kniha_jizd_app`), ne
  superuživatelem.
